...
Finally, we introduce a web interface developed for easy inspection of the audio tracks in the MusicNet dataset, which helped identify the characteristics and imperfections of the music and the annotations in the dataset.
...
Although the experimental results presented above has shown that a marginal improvement by the appended synthesizer component is possible, significant further work is required to prove the practical viability of the idea.
First of all, although the MusicNet dataset is the largest freely available dataset for multiple-instrument music transcription, it contains a non-negligible amount of inaccuracy in the annotations.
This is largely because the annotations were largely obtained by matching each audio track with a MIDI file containing the same piece using dynamic time warping~\cite{thickstun2017musicnet}.
This caused many inaccurate time mappings near the beginning and the end of each track, and there exist a few cases where totally inaccurate annotations continue over extended periods, because the audio and the MIDI file sometimes do not exactly match due to different repetitions or revisions of the music.
