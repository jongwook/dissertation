...
Because it is hard for a recurrent neural network to propagate long-range dependencies through a chain of recurrent connections, specific recurrent units called \emph{long short-term memory (LSTM)} \cite{hochreiter1997lstm} and \emph{gated recurrent unit (GRU)} \cite{cho2014seq2seq} are devised to resolve the problem and are considered essential for recurrent neural networks.
...
\emph{Deep Q-Network (DQN)} \cite{mnih2015dqn} is a neural network model designed for reinforcement learning, which has been successfully applied to automatically playing Atari games \cite{mnih2013atari} and the agent playing the game of Go that surpassed the human level of skill \cite{silver2016alphago}.
...
\emph{Variational autoencoders} (VAE) \cite{kingma2014vae} address this problem by restricting the posterior latent distribution to be Gaussian.
