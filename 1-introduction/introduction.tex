...
In this context, this thesis focuses on improving the machine understanding of music, among many types of sounds, especially on the ability to automatically transcribe music, which largely remains an unsolved problem despite decades of research.
...
Since recently, with the increased capacity of machine learning models and hardware, many deep generative models have been proposed and shown to be capable of processing high-dimensional multimedia data.
...
\begin{enumerate}
	\item Can we use deep learning methods to obtain disentangled representations of musical audio signals, to be used for automatic music transcription?
	\begin{enumerate}
		\item Can deep generative models such as generative adversarial networks (GAN) learn to differentiate the concept of pitch and timbre, so that it can separately control them in its generation?
...
The purpose of this section is to provide those limitations, and it discusses the scope of music that the proposed AMT system can process, the required capability of symbolic music processing, and the need for the perceptual studies regarding the validity of AMT systems.
...
Secondly, it is possible to plug the harmonic-only model into a pipeline consisting of HPSS (harmonic-percussive source separation) and percussion transcription models as an alternative to the comprehensive approach.
...
As mentioned and justified in the statement of problem (Section \ref{sec:statement}), by choosing per-instrument piano rolls as the output of transcription, many structure-level and symbolic-level concerns in music transcription has been excluded from the scope of this study.
...
However, disregarding these few edge cases, we assume that the piano roll notation can convey an objective transcription for all practical purposes, postulating AMT as a mathematical problem which does not require experiments on human subjects.
...
Therfore, this thesis pursue a more tangible goal of achieving a better accuracy in transcribing music that is easy enough for human transcribers but challenging for machines.
...
\subsection{Applications of Automatic Music Transcription}

Many applications of the techniques in the realm of automatic music transcription is on interactive music systems.
\citeA{vercoe1984performer} suggested a quest for a \emph{synthetic performer}, which can listen, perform, and learn in the context of live performance, and automatic \emph{real-time accompaniment} \cite{dannenberg1985accompaniment} based on dynamic programming was one of the first demonstrations of AMT techniques in interactive music systems.
...
AMT system can create the database for query-by-humming \cite{ghias1995humming}, where users can retrieve music by humming an excerpt of the song, even if only audio data is available.
...
These two kinds of generative systems are familiar to computer music artists are referred to as algorithmic composition \cite{fernandez2013ai} and sound synthesis \cite{cook2002synthesis} models.
This thesis defines the term ``generative model'' more specifically, as in Section \ref{sec:definitions}, as a model that can learn the distribution of the provided data and can sample new samples in the original distribution.
...
By learning to generate data, a model has to be able to know the underlying natural features of the distribution of data, which correspond to the musical concepts such as pitch, timbre, and rhythm.
...
\citeA{lecun2016unsupervised} also introduced unsupervised learning as a cake, comparing supervised learning as icing and reinforcement learning as the cherry on the top, by which he meant that generative model needs to predict much larger scale of information but is able to learn the ``common sense''.

\begin{figure}
	\includegraphics[width=\textwidth]{generative-evolution.pdf}
	\caption{Increasingly realistic qualities of the generated faces using Generative Adversarial Networks, 
		taken from \protect\cite{goodfellow2014gan}, \protect\cite{radford2015dcgan}, \protect\cite{liu2016cogan}, and \protect\cite{karras2017pggan}.
	}
...
By doing so, the ultimate objective is to build an end-to-end differentiable that connects the piano roll representation to audio signals, in order to perform automatic music transcription --- obtaining the most likely piano roll representation for given audio.
...
(a) The earlier methods typically use a carefully crafted function to extract features and performed rule-based predictions, (b) whereas the data-driven methods replaces the later part with a machine learning model trainable with a dataset.
...
\subsection{In the Broader Context of AI Research}
