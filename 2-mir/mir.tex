...
\section{Feature Extraction and Common Principles}

\iffalse % TODO? Common techniques and other trends
\begin{itemize}
	\item Feature engineering: standard pipeline + Summary ACF \cite{tolonen2000multipitch}, specmurt \cite{saito2008specmurt}, bispectrum \cite{argenti2011bispectral}, auditory model (Klapuri chapter 8)
	\item joint versus iterative
	\item HMM post-processing / LSTM/RNN decoding \cite{ewert2017admm}
	\item smoothness principle
	\item music language models as prior
	\item note modeling
	\begin{itemize}
		\item onset/offset, attack/decay, temporal evolution
	\end{itemize}
\end{itemize}
\fi
...
Once an appropriate set of features are obtained, the next steps for MIR algorithms typically involve applying a classification model, probabilistic inference, or other task-specific algorithms.
% Examples of classification models include random forests, support vector machines (SVM), and Gaussian mixture model (GMM) classifiers that have been applied to music tagging \cite{ness2009tag}, melody extraction \cite{bittner2015contour}, and genre classification \cite{tzanetakis2002genre} tasks.

% smoothness principle
\textit{Hidden Markov models (HMM)} are widely employed for modeling sequence data such as chord progressions \cite{cho2010chord} as well as to smooth the sequence output as a post-processing step \cite{khadkevich2009hmm}, often paired with Gaussian mixture models as the emission probability distribution.
...
Due to the simplicity of the algorithm and the applicability of the non-negative setup to magnitude spectrograms, many MFFE algorithms using NMF have been introduced.
\citeA{smaragdis2003nmf} showed that NMF can extract the temporal and spectral profiles of piano notes even when the notes are being played concurrently.
The NMF model was extended by incorporating sparsity loss \cite{cont2006realtime,costantini2013nmf}, non-negative matrix division \cite{niedermayer2008division}, $\beta$-divergence \cite{dessein2010beta}, and a Bayesian framework for encouraging harmonicity and temporal smoothness \cite{bertin2009nmf,bertin2010nmf,peeling2010factorization}.
With carefully designed regularizers and the \textit{alternating direction method of multipliers (ADMM)} for training NMF, \citeA{ewert2016admm,ewert2017admm} achieved close-to-perfect accuracy for piano transcription in a studio setting.
...
Advances in deep learning enabled neural networks to serve an alternative to SVMs for discriminative transcription.
...
a dual-objective loss for onsets and frames \cite{hawthorne2018piano},
